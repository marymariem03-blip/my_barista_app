...
import 'barista_dashboard_screen.dart';
import 'barista_orders_screen.dart';
import 'barista_profile_screen.dart';

class BaristaMainScreen extends StatefulWidget {
  const BaristaMainScreen({super.key});

  @override
  State<BaristaMainScreen> createState() => _BaristaMainScreenState();
}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: IndexedStack(
        index: _tab,
        children: const [
          BaristaDashboardScreen(),
          BaristaOrdersScreen(),
          BaristaProfileScreen(),
        ],
      ),
        ...
        child: Row(
          ...
          children: [
            _NavItem(icon: Icons.dashboard_outlined,
                label: 'Dashboard', isActive: _tab == 0,
                onTap: () => setState(() => _tab = 0)),
            _NavItem(icon: Icons.receipt_long_outlined,
                label: 'Commandes', isActive: _tab == 1,
                onTap: () => setState(() => _tab = 1)),
            _NavItem(icon: Icons.person_outline,
                label: 'Profil', isActive: _tab == 2,
